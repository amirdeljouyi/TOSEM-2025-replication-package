...
public class ResourceList {
    ...
    public String hasCandidate(String className) {
        // typically if it ends with className, like Foo in com.examples.with.different.packagename.classpath.Foo
        for (String key : getCache().mapClassToCP.keySet()) {
     ...
     * @return
     */
    public String hasCandidateAllPossibilities(String className) {
        // typically if it ends with className, like Foo in com.examples.with.different.packagename.classpath.Foo
        String result = hasCandidate("." + className);

        if(result == null) {
            // go to replace . with $ to find the last one
            if(className.contains(".")){
                String newClassName = replaceLastOccurrence(className, ".", "$");
                result = hasCandidate("." + newClassName);
            }
        }

        if(result == null) {
            // if it doesn't have . but it ends with $ (last option)
            result = hasCandidate("$" + className);
        }
