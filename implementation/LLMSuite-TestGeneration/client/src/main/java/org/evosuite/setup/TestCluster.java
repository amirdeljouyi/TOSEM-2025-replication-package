...
//					}
    ...
    public Class<?> getClass(String name) throws ClassNotFoundException {
        ...
        // Or try java.lang
        try {
            return TestGenerationContext.getInstance().getClassLoaderForSUT().loadClass("java.lang." + name);
        } catch (ClassNotFoundException e) {
            // Ignore it as we throw our own
        }

        // Or try java.util -> ArrayList
        try {
            return TestGenerationContext.getInstance().getClassLoaderForSUT().loadClass("java.util." + name);
        } catch (ClassNotFoundException e) {
            // Ignore it as we throw our own
        }
        ...
        // Or try java.io
        try {
            return TestGenerationContext.getInstance().getClassLoaderForSUT().loadClass("java.io." + name);
        } catch (ClassNotFoundException e) {
            // Ignore it as we throw our own
        }

        // try with a qualified name
        try {
            return resolveClassNotSeen(name);
        } catch (ClassNotFoundException e) {
            // Ignore it as we throw our own
        }

        // try with a simple name
        try {
            String clazzName = ResourceList.getInstance(TestGenerationContext.getInstance().getClassLoaderForSUT()).hasCandidateAllPossibilities(name);
            return resolveClassNotSeen(clazzName);
        } catch (ClassNotFoundException e) {
            // try with simpler class name
            LoggingUtils.getEvoLogger().warn("** Could not find class " + name, e);
        }
    ...
    private Class<?> resolveClassNotSeen(String className) throws ClassNotFoundException {
        if (className != null) {
            Class<?> clazz = TestGenerationContext.getInstance().getClassLoaderForSUT().loadClass(className);
            LoggingUtils.getEvoLogger().info("** Could find class " + className);
            if (clazz != null) {
     ...
     * @throws ConstructionFailedException
     */
    public Set<GenericAccessibleObject<?>> getGenerators(GenericClass<?> clazz) throws
