...
//				return new MOSA(factory);
            ...
            case CODAMOSA:
                ...
                if (factory instanceof TestSuiteChromosomeFactory) {
                    final TestSuiteChromosomeFactory tscf = (TestSuiteChromosomeFactory) factory;
                    return new MOSATestSuiteAdapter(new CodaMOSA(tscf.getTestChromosomeFactory()));
                } else {
                    logger.info("No specific factory for test cases given...");
                    logger.info("Using a default factory that creates tests with variable length");
                    return new MOSATestSuiteAdapter(new CodaMOSA(new RandomLengthTestFactory()));
                }
            case DYNAMOSA:
...
//				return new DynaMOSA(factory);
            ...
            case LLM_DYNAMOSA:
                logger.info("Chosen search algorithm: LLMDynaMOSA");
                LoggingUtils.getEvoLogger().info("Chosen search algorithm: LLMDynaMOSA");
                if (factory instanceof TestSuiteChromosomeFactory) {
                    final TestSuiteChromosomeFactory tscf = (TestSuiteChromosomeFactory) factory;
                    return new MOSATestSuiteAdapter(new LLMDynaMOSA(tscf.getTestChromosomeFactory()));
                } else {
                    logger.info("No specific factory for test cases given...");
                    logger.info("Using a default factory that creates tests with variable length");
                    return new MOSATestSuiteAdapter(new LLMDynaMOSA(new RandomLengthTestFactory()));
                }
