/**
 * Scaffolding file used to store all the setups needed to run 
 * tests automatically generated by EvoSuite
 * Mon Jun 23 13:03:31 GMT 2025
 */

package cc.mallet.fst;

import org.evosuite.runtime.annotation.EvoSuiteClassExclude;
import org.junit.BeforeClass;
import org.junit.Before;
import org.junit.After;

import static org.evosuite.shaded.org.mockito.Mockito.*;
@EvoSuiteClassExclude
public class SumLatticeBeam_4_ESTest_scaffolding {

  @org.junit.Rule
  public org.evosuite.runtime.vnet.NonFunctionalRequirementRule nfr = new org.evosuite.runtime.vnet.NonFunctionalRequirementRule();

  private org.evosuite.runtime.thread.ThreadStopper threadStopper =  new org.evosuite.runtime.thread.ThreadStopper (org.evosuite.runtime.thread.KillSwitchHandler.getInstance(), 3000);


  @BeforeClass
  public static void initEvoSuiteFramework() { 
    org.evosuite.runtime.RuntimeSettings.className = "cc.mallet.fst.SumLatticeBeam"; 
    org.evosuite.runtime.GuiSupport.initialize(); 
    org.evosuite.runtime.RuntimeSettings.maxNumberOfThreads = 100; 
    org.evosuite.runtime.RuntimeSettings.maxNumberOfIterationsPerLoop = 10000; 
    org.evosuite.runtime.RuntimeSettings.mockSystemIn = true; 
    org.evosuite.runtime.Runtime.getInstance().resetRuntime(); 
    try { initMocksToAvoidTimeoutsInTheTests(); } catch(ClassNotFoundException e) {} 
  } 

  @Before
  public void initTestCase(){ 
    threadStopper.storeCurrentThreads();
    threadStopper.startRecordingTime();
    org.evosuite.runtime.jvm.ShutdownHookHandler.getInstance().initHandler(); 
    org.evosuite.runtime.GuiSupport.setHeadless(); 
    org.evosuite.runtime.Runtime.getInstance().resetRuntime(); 
    org.evosuite.runtime.agent.InstrumentingAgent.activate(); 
  } 

  @After
  public void doneWithTestCase(){ 
    threadStopper.killAndJoinClientThreads();
    org.evosuite.runtime.jvm.ShutdownHookHandler.getInstance().safeExecuteAddedHooks(); 
    org.evosuite.runtime.agent.InstrumentingAgent.deactivate(); 
    org.evosuite.runtime.GuiSupport.restoreHeadlessMode(); 
  } 

  public static void setSystemProperties() {
 
    /*No java.lang.System property to set*/
  }

  private static void initializeClasses() {
    org.evosuite.runtime.classhandling.ClassStateSupport.initializeClasses(SumLatticeBeam_4_ESTest_scaffolding.class.getClassLoader() ,
      "cc.mallet.optimize.Optimizer",
      "cc.mallet.fst.SumLatticeFactory",
      "cc.mallet.fst.TransducerTrainer$ByIncrements",
      "cc.mallet.types.Multinomial$MAPEstimator",
      "cc.mallet.classify.BalancedWinnowTrainer",
      "cc.mallet.fst.HMM",
      "cc.mallet.fst.MaxLatticeDefault$Factory",
      "cc.mallet.util.CharSequenceLexer",
      "com.carrotsearch.hppc.ObjectCollection",
      "cc.mallet.types.InstanceList$CrossValidationIterator",
      "cc.mallet.types.Labeling",
      "cc.mallet.pipe.CharSequenceReplace",
      "cc.mallet.pipe.AddClassifierTokenPredictions$TokenClassifiers",
      "cc.mallet.types.RankedFeatureVector",
      "cc.mallet.types.Dirichlet",
      "cc.mallet.types.DenseVector",
      "cc.mallet.types.Sequence",
      "cc.mallet.fst.SumLatticeBeam$Factory",
      "cc.mallet.fst.Segment",
      "com.carrotsearch.hppc.RandomizedHashOrderMixer",
      "cc.mallet.fst.FeatureTransducer$TransitionIterator",
      "cc.mallet.fst.SumLatticeBeam$NBForBackNode",
      "cc.mallet.types.Multinomial$LaplaceEstimator",
      "cc.mallet.pipe.Token2FeatureVector",
      "com.carrotsearch.hppc.Containers$1",
      "cc.mallet.fst.MEMM",
      "cc.mallet.pipe.SerialPipes",
      "cc.mallet.optimize.Optimizable",
      "com.carrotsearch.hppc.HashOrderMixing$Strategy$2",
      "com.carrotsearch.hppc.Containers",
      "com.carrotsearch.hppc.HashOrderMixing$Strategy$3",
      "cc.mallet.classify.Boostable",
      "cc.mallet.types.TokenSequence",
      "com.carrotsearch.hppc.HashOrderMixing$Strategy$1",
      "cc.mallet.fst.CRFOptimizableByBatchLabelLikelihood",
      "cc.mallet.types.Multinomial$MLEstimator",
      "cc.mallet.fst.CRF$Factors$Incrementor",
      "cc.mallet.types.Multinomial$Estimator",
      "cc.mallet.fst.MaxLattice",
      "cc.mallet.types.SequencePair",
      "cc.mallet.pipe.TokenSequenceRemoveStopwords",
      "com.carrotsearch.hppc.HashOrderMixing",
      "com.carrotsearch.hppc.BitMixer",
      "cc.mallet.fst.MEMM$State",
      "com.carrotsearch.hppc.ObjectContainer",
      "cc.mallet.fst.FeatureTransducer",
      "cc.mallet.fst.MaxLatticeFactory",
      "com.carrotsearch.hppc.ObjectIntHashMap",
      "cc.mallet.fst.TransducerTrainer",
      "cc.mallet.fst.CRF",
      "cc.mallet.fst.CRFTrainerByStochasticGradient",
      "cc.mallet.util.Lexer",
      "cc.mallet.types.Token",
      "cc.mallet.pipe.TargetStringToFeatures",
      "com.carrotsearch.hppc.HashContainers",
      "cc.mallet.types.NullLabel",
      "cc.mallet.types.Vector",
      "cc.mallet.fst.TransducerEvaluator",
      "cc.mallet.types.Multinomial",
      "cc.mallet.classify.Classification",
      "cc.mallet.fst.FeatureTransducer$Transition",
      "cc.mallet.types.LabelAlphabet",
      "cc.mallet.fst.CRF$Factors$WeightedIncrementor",
      "com.google.errorprone.annotations.Var",
      "cc.mallet.pipe.PrintInputAndTarget",
      "cc.mallet.pipe.SimpleTaggerSentence2TokenSequence",
      "cc.mallet.types.PropertyHolder",
      "com.carrotsearch.hppc.IntObjectHashMap",
      "cc.mallet.types.Matrix",
      "com.carrotsearch.hppc.BitUtil",
      "cc.mallet.pipe.Target2Label",
      "cc.mallet.types.FeatureSequence",
      "com.carrotsearch.hppc.HashOrderMixingStrategy",
      "cc.mallet.fst.SumLatticeBeam$Factory$1",
      "cc.mallet.util.MalletLogger",
      "cc.mallet.classify.Classifier",
      "cc.mallet.types.ArrayListSequence",
      "cc.mallet.fst.CRFTrainerByThreadedLabelLikelihood",
      "cc.mallet.types.InstanceList",
      "cc.mallet.pipe.Pipe",
      "cc.mallet.types.LabelSequence",
      "cc.mallet.util.Randoms$1",
      "com.carrotsearch.hppc.HashOrderMixing$Strategy",
      "cc.mallet.fst.HMM$State",
      "com.carrotsearch.hppc.BufferAllocationException",
      "cc.mallet.util.Randoms",
      "cc.mallet.types.SparseVector",
      "cc.mallet.types.FeatureInducer",
      "cc.mallet.types.FeatureVector",
      "cc.mallet.types.PagedInstanceList",
      "cc.mallet.pipe.AddClassifierTokenPredictions",
      "cc.mallet.fst.SumLatticeBeam$NBestSlist",
      "cc.mallet.fst.SumLatticeDefault$Factory",
      "cc.mallet.fst.CRF$State",
      "com.carrotsearch.hppc.IntContainer",
      "cc.mallet.types.MatrixOps",
      "cc.mallet.fst.HMM$WeightedIncrementor",
      "cc.mallet.classify.ClassifierTrainer",
      "cc.mallet.types.FeatureVectorSequence",
      "cc.mallet.fst.HMMTrainerByLikelihood",
      "cc.mallet.types.AlphabetCarrying",
      "cc.mallet.fst.HMM$TransitionIterator",
      "cc.mallet.pipe.Target2Integer",
      "cc.mallet.fst.Transducer$State",
      "cc.mallet.types.InstanceList$NotYetSetPipe",
      "cc.mallet.pipe.SaveDataInSource",
      "cc.mallet.fst.FeatureTransducer$State",
      "cc.mallet.fst.SumLatticeBeam$LatticeNode",
      "cc.mallet.pipe.TokenSequenceRemoveNonAlpha",
      "cc.mallet.pipe.Noop",
      "cc.mallet.optimize.Optimizable$ByCombiningBatchGradient",
      "cc.mallet.fst.Transducer",
      "cc.mallet.fst.CRF$Factors",
      "com.carrotsearch.hppc.ObjectIntMap",
      "cc.mallet.types.SequencePairAlignment",
      "cc.mallet.types.Alphabet",
      "cc.mallet.fst.HMM$Incrementor",
      "cc.mallet.types.Instance",
      "cc.mallet.types.LabelSequence$Iterator",
      "cc.mallet.types.FeatureSelection",
      "cc.mallet.util.PropertyList",
      "com.carrotsearch.hppc.IntObjectAssociativeContainer",
      "cc.mallet.fst.Transducer$Incrementor",
      "cc.mallet.fst.TransducerTrainer$ByInstanceIncrements",
      "cc.mallet.fst.TransducerTrainer$ByOptimization",
      "com.carrotsearch.hppc.ObjectIntAssociativeContainer",
      "cc.mallet.fst.CacheStaleIndicator",
      "cc.mallet.types.Label",
      "com.carrotsearch.hppc.Preallocable",
      "cc.mallet.types.Multinomial$MEstimator",
      "cc.mallet.fst.SumLatticeBeam",
      "cc.mallet.fst.Transducer$TransitionIterator",
      "com.carrotsearch.hppc.IntCollection",
      "cc.mallet.fst.MEMM$TransitionIterator",
      "cc.mallet.types.PagedInstanceList$PagedInstanceListIterator",
      "cc.mallet.types.ConstantMatrix",
      "com.carrotsearch.hppc.HashOrderMixing$1",
      "com.carrotsearch.hppc.IntObjectMap",
      "cc.mallet.pipe.Array2FeatureVector",
      "cc.mallet.types.DenseMatrix",
      "cc.mallet.fst.CRF$TransitionIterator",
      "cc.mallet.types.LabelVector",
      "cc.mallet.fst.SumLattice",
      "cc.mallet.optimize.Optimizable$ByGradientValue",
      "com.carrotsearch.hppc.HashOrderMixing$4"
    );
  } 
  private static void initMocksToAvoidTimeoutsInTheTests() throws ClassNotFoundException { 
    mock(Class.forName("cc.mallet.fst.Transducer", false, SumLatticeBeam_4_ESTest_scaffolding.class.getClassLoader()));
    mock(Class.forName("cc.mallet.fst.Transducer$State", false, SumLatticeBeam_4_ESTest_scaffolding.class.getClassLoader()));
    mock(Class.forName("cc.mallet.fst.Transducer$TransitionIterator", false, SumLatticeBeam_4_ESTest_scaffolding.class.getClassLoader()));
    mock(Class.forName("cc.mallet.types.Sequence", false, SumLatticeBeam_4_ESTest_scaffolding.class.getClassLoader()));
    mock(Class.forName("java.util.function.Consumer", false, SumLatticeBeam_4_ESTest_scaffolding.class.getClassLoader()));
  }
}
